...
installation. Do not set ComfyCopyToPlugins during verification.
    ...
    $contractsHash = (Get-FileHash packages-local/Comfy.Quest.Contracts.0.9.6-local.nupkg -Algorithm SHA256).Hash.ToLowerInvariant().Substring(0,16)
...
The Creator/DM authoring cutover has its own `0.9.6-local` Contracts line. Never repack
