...
The [Derek checklist](docs/derek-checklist.md) records the AM4 create, play, create,
revise walkthrough, configurable practice profiles, native gameplay evidence, and
release handoff. The earlier [connected campaign handoff](docs/creator-connected-campaign.md)
remains historical evidence. Live controls, gameplay and human judgment are recorded separately.
...
installation. Do not set ComfyCopyToPlugins during verification.
    ...
    $contractsHash = (Get-FileHash packages-local/Comfy.Quest.Contracts.0.9.13-local.nupkg -Algorithm SHA256).Hash.ToLowerInvariant().Substring(0,16)
...
The Creator/DM authoring cutover has its own `0.9.13-local` Contracts line. Never repack
