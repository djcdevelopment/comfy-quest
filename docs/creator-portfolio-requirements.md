...
| Studio document | [`StudioProjectDocument`](../src/Quest.Studio/QuestStudioWorkspace.cs) schema 3 owns one pack id, one experience id, one title, bindings, one entry node, and a node list. Project summaries form a flat library. `BuildPack` writes exactly one `experiences/<id>.json`. | No portfolio hierarchy, guild identity, questline, cross-quest progression, repeatable-event policy, named-anchor authoring, or batch release view exists. |
...
| Runtime progress | [`WorkflowIdentity.Key`](../network/mod/ComfyQuestContracts/WorkflowRuntime.cs) is world + character + binding ZDO + content hash. `WorkflowStateStore` persists terminal outcomes and ignores later events; timers, action claims, and spawned-object ownership live in separate durable stores. | There is no scoped retry/reset operation. Restoring only a world or only `workflow-states.json` cannot safely make a run clean. |
| Studio operations | The [v2 endpoints](../src/Quest.Studio/QuestStudioEndpoints.cs) create, import-fork, duplicate, save with revision guards, validate, certify, rehearse, play a dev revision, publish, inspect history/status, and export. The only reset endpoint resets local usage data. | The creator cannot start a clean run, resume a run intentionally, preview reset blast radius, or reset one quest/event from Studio. |
...
| Evidence | [`RuntimeReceiptStore`](../network/mod/ComfyQuestContracts/RuntimeReceipts.cs) writes correlated JSON receipts and lists at most 200 per read. The receipt directory itself has no retention policy. Rehearsal declares its proof level and limitations. | Guild-scale iteration needs run grouping, coverage, creator-altitude explanations, export, and bounded retention without weakening raw evidence. |
