...
| 2026-08-28T04:29Z, unattended ERA17 entry r8 | The pinned `tugcorp` profile was reported missing before world load, although Prepare had just snapshotted a Steam-cloud file with that name and r7 had entered with it. | World-entry receipt `ddc205bc5c4548c9bb96677a7eedb694`; r8 request; r8 `Player.log` (`Zephar410`, account ID `15896021`, no `tugcorp` in platform saves); r7 `Player-prev.log` (`wary.fool`, account ID `128445914`, `tugcorp` present and loaded); Steam `ActiveProcess\ActiveUser`; the two account-scoped userdata roots. | **ANSWERED by the bounded comparison.** Prepare recursively searched all Steam accounts and accepted another account's profile; Runtime correctly saw only the active account's platform saves. Waiting longer cannot make that other account's inventory visible. | Restrict Prepare to the active Steam account, persist its non-secret account ID, and make Launch refuse any account drift before creating the one-shot request. Do not automate credentials or mutate Steam's account configuration. |
| 2026-08-28T04:41Z, r9 binding discovery | Studio reported the exact project/world connected with no run, then the first candidate-scan POST returned HTTP 400 and created no Runtime mailbox or receipt. | Studio access log preserves the 200 run-status read followed by the 400 candidate POST; Runtime had no pending request or new receipt; the later `runs.json` heartbeat was 6.4 seconds old. | The PowerShell caller discarded the typed error response body. The evidence is consistent with Studio's strict `runtime_run_status_stale` precondition, but cannot distinguish it from another pre-dispatch Studio refusal. | Use the already packaged Quest workbench provider for one correlated `list_binding_candidates` request and preserve its Runtime receipt. Separately, make any future HTTP driver retain non-2xx bodies before claiming a Studio cause. |
