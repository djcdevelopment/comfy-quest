...
- `Comfy.Quest.Contracts` and `Comfy.Quest.Studio` producer version
  `0.4.0-local`;
- every Contracts consumer pinned exactly to `[0.4.0-local]`; and
...
```powershell
git tag nuget-v0.4.0
git push origin nuget-v0.4.0
```
...
```powershell
python tools\nuget\repin_public.py --apply --version 0.4.0
python tools\nuget\repin_public.py --check --version 0.4.0
