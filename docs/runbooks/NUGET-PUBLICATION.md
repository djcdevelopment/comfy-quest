...
- `Comfy.Quest.Contracts` and `Comfy.Quest.Studio` producer version
  `0.9.9-local`;
- every Contracts consumer pinned exactly to `[0.9.9-local]`; and
...
```powershell
git tag nuget-v0.9.3
git push origin nuget-v0.9.3
```
...
```powershell
python tools\nuget\repin_public.py --apply --version 0.9.3
python tools\nuget\repin_public.py --check --version 0.9.3
