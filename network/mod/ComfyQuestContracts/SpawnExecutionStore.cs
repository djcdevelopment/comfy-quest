...
public sealed class SpawnExecutionStore {
  ...
  public bool CanRecord(int additional,out string error){lock(gate){var state=Read();if(state.Unreadable){error="spawn_state_unreadable";return false;}if(additional<1||additional>16||state.Objects.Count+additional>MaxObjects){error="spawn_state_limit";return false;}var currentBytes=File.Exists(path)?new FileInfo(path).Length:0L;if(currentBytes+additional*2048L>MaxStateBytes){error="spawn_state_too_large";return false;}error=null;return true;}}
