...
public sealed class WorkflowStateStore {
  ...
  static TriggerEvaluationContext Context(WorkflowProgress state,DateTimeOffset at,ExperienceDocument document,SpatialFacts spatial,EncounterFacts encounter)=>new(){At=at,StageEnteredUtc=state.StageEnteredUtc,LastProgressUtc=state.LastProgressUtc,BindingPosition=spatial?.BindingPosition,SpawnedPositions=spatial?.SpawnedPositions,AuthoredAnchors=SpatialEvaluator.AnchorMap(document),DeathsInStage=state.DeathsInStage,SpawnsByAction=encounter?.SpawnsByAction};
