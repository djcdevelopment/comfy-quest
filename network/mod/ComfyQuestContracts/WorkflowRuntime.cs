...
public sealed class WorkflowStateStore {
  ...
  public WorkflowDecision Begin(WorkflowIdentity identity,ExperienceDocument document,RuntimeEvent evt){if(identity==null||document==null||evt==null)throw new ArgumentNullException();lock(gate){var file=Read();if(file.Unreadable)return null;if(!file.States.TryGetValue(identity.Key,out var state)){state=new WorkflowProgress{StageId=document.EntryStage,StageEnteredUtc=evt.At,LastEventUtc=evt.At};file.States[identity.Key]=state;}if(!string.IsNullOrWhiteSpace(state.Outcome))return null;var stage=document.Stages?.FirstOrDefault(x=>x.Id==state.StageId);if(stage==null)return null;if(!string.IsNullOrWhiteSpace(state.PendingTransitionId)){var pending=stage.Transitions?.FirstOrDefault(x=>x.Id==state.PendingTransitionId);var eventAt=state.LastEventUtc??state.History.LastOrDefault()?.At;return pending==null||!eventAt.HasValue?null:new(){Identity=identity,StageId=stage.Id,Transition=pending,IsPendingReplay=true,EventAt=eventAt};}state.LastEventUtc=evt.At;state.History.Add(evt);if(state.History.Count>MaxHistory)state.History.RemoveRange(0,state.History.Count-MaxHistory);var transition=(stage.Transitions??new()).OrderByDescending(x=>x.Priority).ThenBy(x=>x.Id,StringComparer.Ordinal).FirstOrDefault(x=>TriggerEvaluator.Matches(x.When,state.History));if(transition==null){Write(file);return null;}state.PendingTransitionId=transition.Id;state.PendingFromStageId=stage.Id;state.PendingNextStageId=transition.NextStage;state.PendingOutcome=transition.Outcome;Write(file);return new(){Identity=identity,StageId=stage.Id,Transition=transition,EventAt=evt.At};}}
  public bool Complete(WorkflowDecision decision){if(decision?.Identity==null||decision.Transition==null)return false;lock(gate){var file=Read();if(file.Unreadable||!file.States.TryGetValue(decision.Identity.Key,out var state)||state.PendingTransitionId!=decision.Transition.Id||state.StageId!=decision.StageId)return false;var nextStage=string.IsNullOrWhiteSpace(state.PendingNextStageId)?state.StageId:state.PendingNextStageId;var advances=!string.Equals(state.StageId,nextStage,StringComparison.Ordinal);if(advances&&!decision.EventAt.HasValue)return false;if(advances)state.StageEnteredUtc=decision.EventAt.Value;state.StageId=nextStage;state.Outcome=state.PendingOutcome;state.History.Clear();state.PendingTransitionId=null;state.PendingFromStageId=null;state.PendingNextStageId=null;state.PendingOutcome=null;Write(file);return true;}}
