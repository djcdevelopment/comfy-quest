  ...
  public IReadOnlyList<RuntimeBindingCandidate> ListCandidates(){
    ...
    for(var index=0;index<count;index++){
      ...
      values.Add(new RuntimeBindingCandidate{BindingZdo=zdo.m_uid.ToString(),TargetKind=kind,
        ...
        Prefab=prefab?.name??("hash:"+zdo.GetPrefab()),Position=new RuntimeBindingPosition{
...
sealed class RuntimeRunControlController {
  ...
  readonly string packRoot,requestPath,receiptDirectory;readonly RuntimeExperienceEngine engine;readonly RuntimeReceiptStore runtimeReceipts;readonly Func<bool> privateConfirmed,worldLoaded;readonly Func<string> worldUid,creatorSessionId;readonly Action<string> log;readonly IRuntimeSpawnResetAdapter adapter;readonly RuntimeBindingCoordinator bindings;double nextPoll;
  public RuntimeRunControlController(string runtimeRoot,RuntimeExperienceEngine runtimeEngine,RuntimeReceiptStore receipts,Func<bool> isPrivateConfirmed,Func<bool> isWorldLoaded,Func<string> currentWorldUid,Func<string> currentCreatorSessionId,Action<string> logger=null,IRuntimeSpawnResetAdapter spawnAdapter=null,IRuntimeBindingAdapter bindingAdapter=null){var root=Path.GetFullPath(runtimeRoot??throw new ArgumentNullException(nameof(runtimeRoot)));engine=runtimeEngine??throw new ArgumentNullException(nameof(runtimeEngine));runtimeReceipts=receipts??throw new ArgumentNullException(nameof(receipts));privateConfirmed=isPrivateConfirmed??(()=>false);worldLoaded=isWorldLoaded??(()=>false);worldUid=currentWorldUid??(()=>string.Empty);creatorSessionId=currentCreatorSessionId??throw new ArgumentNullException(nameof(currentCreatorSessionId));log=logger??(_=>{});adapter=spawnAdapter??new RuntimeSpawnResetAdapter();bindings=new RuntimeBindingCoordinator(root,bindingAdapter??new RuntimeBindingWorldAdapter(),engine.RunCoordinator().Registry);packRoot=root;requestPath=Path.Combine(root,"requests","run-control.json");receiptDirectory=RuntimeRunControlReceipts.Root(root);}
  ...
  void Dispatch(RuntimeRunControlRequest request){
    ...
    // Restore is a fail-safe rollback of one exact recorded binding transaction. Every other
    // observation or mutation belongs to the Creator Session that actually completed world entry.
    if(request.Operation!="restore_binding"){
      var activeSession=creatorSessionId();
      if(string.IsNullOrWhiteSpace(activeSession)){Write(request,"rejected","runtime_creator_session_unavailable",null,null);return;}
      if(request.CreatorSessionId!=activeSession){Write(request,"rejected","runtime_creator_session_mismatch",null,null);return;}
    }
    ...
    try{
      ...
      if(request.Operation=="preview_retire"){var preview=engine.RunCoordinator().PreviewRetire(request.RunId,DateTimeOffset.UtcNow,adapter);Write(request,"previewed","retire_preview_ready",null,null,null,null,preview,null);return;}
      if(!privateConfirmed()){Write(request,"rejected","private_world_confirmation_required",null,null);return;}
      if(request.Operation=="apply_retire"){
        var retired=engine.RunCoordinator().ApplyRetire(request.RunId,request.PreviewToken,DateTimeOffset.UtcNow,
          adapter,()=>engine.ClearRunEphemera(run.StateKey));
        Write(request,retired.State=="completed"?"completed":"failed",retired.Detail??retired.State,null,null,null,null,null,retired);return;
      }
