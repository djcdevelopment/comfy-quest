...
This directory holds the connected Creator/DM 0.9.11-local NuGet packages so the
repository can restore without reaching into another checkout. Historical
0.9.9-local and 0.9.10-local packages remain byte-for-byte unchanged. All versions
are immutable.
