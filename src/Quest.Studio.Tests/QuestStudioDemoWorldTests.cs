...
public sealed class QuestStudioDemoWorldTests : IDisposable
{
    ...
    public void Tutorial_expectations_pin_exact_activation_cast_rebind_receipts_and_demo_world_landmarks()
    {
        ...
        var guidance = target.GetProperty("guidance_path");
        Assert.Equal("itemstandh", guidance.GetProperty("breadcrumb_prefab").GetString());
        Assert.Equal(3, guidance.GetProperty("breadcrumb_count").GetInt32());
        Assert.Equal("arcane_sight_only", guidance.GetProperty("breadcrumb_visual").GetString());
        Assert.Equal("piece_groundtorch", guidance.GetProperty("destination_prefab").GetString());
        Assert.True(guidance.GetProperty("infinite_fuel").GetBoolean());
        Assert.True(guidance.GetProperty("walking_lane_remains_clear").GetBoolean());
        ...
        Assert.Equal(0, world.GetProperty("unavoidable_ascent_portal").GetProperty("upper_portal_yaw").GetInt32());
        Assert.True(world.GetProperty("unavoidable_ascent_portal").GetProperty("arrival_faces_binding_target").GetBoolean());
