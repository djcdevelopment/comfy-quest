...
KNOWN_LOCAL_FILES = {
    "Comfy.Quest.Contracts.0.9.6-local.nupkg",
    "Comfy.Quest.Studio.0.9.6-local.nupkg",
    "README.md",
}
...
def check_public_state(version: str) -> None:
    ...
    for path, text in (
        ...
        if "0.9.6-local" in text or "packages-local" in text:
...
def apply(version: str) -> None:
    ...
    studio = replace_once(
        studio,
        ">[0.9.6-local]</QuestContractsVersion>",
...
def main() -> int:
    ...
    try:
        if args.apply:
            ...
            check_interim_state()
            print("VERIFIED exact 0.9.6-local interim pins and local-first feed")
        else:
